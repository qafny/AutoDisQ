...
Fixpoint intersect_locus' (x : range) (y : locus) : bool :=
  match y with
  | [] => false
  | a :: yas =>
      if same_name x a then
        if nat_range_inter (snd x) (snd a) then true else intersect_locus' x yas
      else
        intersect_locus' x yas
  end.

Fixpoint intersect_locus (x y : locus) : bool :=
  match x with
  | [] => false
  | a :: xas => if intersect_locus' a y then true else intersect_locus xas y
  end.
...
Print  DisQ.DisQSyntax.range.
Print DisQSyntax.locus.

Definition get_locus_cexp (a : cexp) : locus :=
  match a with
  | CNew x n     => [(x,(0,n))]
  | CAppU l _e   => l
  | CMeas _x k   => k
...
Definition get_loci (xs : list cexp) : locus :=
  fold_left (fun acc a => get_locus_cexp a ++ acc) xs [].

Definition get_locus (op : myOp) : locus :=
  match op with
  | OpAP a       => get_locus_cexp a
  | OpIf _b p q  => get_loci (p ++ q)
  end.
...
Definition is_inter (x y : cexp) : bool :=
  intersect_locus (get_locus_cexp x) (get_locus_cexp y).
(*
Definition is_inter (x y : cexp) : bool :=
  let la := get_locus_cexp x in
  let lb := get_locus_cexp y in
  intersect la lb.*)
(*
Definition is_inter (x y: cexp) :=
  match get_locus x, get_locus y with la,lb => intersect la lb end.
...
Definition vars_of_range (r : range) : list var := [fst r].
...
Definition get_vars_cexp (c : cexp) : list var :=
  match c with
  | CNew q _n     => [q]
  | CAppU l e     => vars_of_locus l ++ vars_of_exp e
  | CMeas x k     => [x] ++ vars_of_locus k
  end.

Definition gen_hb_single (x y : nat * myOp) (acc : hb_relation) : hb_relation :=
  ...
  | OpAP xa, OpIf ya la ra =>
      fun i j =>
        if (i =? fst x) && (j =? fst y)
        then  intersect_locus (get_locus_cexp xa) (get_loci (la ++ ra))
  ...
  | OpIf xa la ra, OpIf ya lb rb =>
      fun i j =>
        if (i =? fst y) && (j =? fst x)
        then  intersect_locus (get_loci (la ++ ra)) (get_loci (lb ++ rb))
...
Definition cexp_eqb (a b : cexp) : bool :=
  match a, b with
  | CNew x1 n1, CNew x2 n2 =>
      Nat.eqb x1 x2 && Nat.eqb n1 n2

  | CAppU l1 e1, CAppU l2 e2 =>
      locus_eqb l1 l2 && exp_eqb e1 e2   (* assumes exp_eqb exists *)

  | CMeas x1 k1, CMeas x2 k2 =>
      Nat.eqb x1 x2 && locus_eqb k1 k2
...
Definition cdexp_eqb (d1 d2 : cdexp) : bool :=
  ...
  | NewCh c1 n1, NewCh c2 n2 =>
      Nat.eqb c1 c2 && Nat.eqb n1 n2
  | Send c1 a1, Send c2 a2 =>
      Nat.eqb c1 c2 && aexp_eqb a1 a2
  | Recv c1 x1, Recv c2 x2 =>
      Nat.eqb c1 c2 && Nat.eqb x1 x2
  | _, _ => false
  end.
...
Definition range_eqb (r1 r2 : range) : bool :=
  ...
  | ((x1, lo1), hi1), ((x2, lo2), hi2) =>
      andb (Nat.eqb x1 x2)
        (andb
          (match lo1, lo2 with
           | BNum n1, BNum n2 => Nat.eqb n1 n2
           | BVar v1 o1, BVar v2 o2 => andb (Nat.eqb v1 v2) (Nat.eqb o1 o2)
           | _, _ => false
           end)
          (match hi1, hi2 with
           | BNum n1, BNum n2 => Nat.eqb n1 n2
...
Definition leb (x y : range) : bool :=
  let '((qx,lox), hix) := x in
  let '((qy,loy), hiy) := y in
  if Nat.ltb qx qy then true
  else if Nat.eqb qx qy then
    match lox, loy with
    | BNum n1, BNum n2 => Nat.leb n1 n2
    | BVar v1 o1, BVar v2 o2 =>
        if Nat.ltb v1 v2 then true
        else if Nat.eqb v1 v2 then Nat.leb o1 o2
        else false
    | _, _ => false
    end
  else false.
...
Definition nat_range_inter (x y : (nat * nat)) :=
      ((fst x <=? fst y) && (fst y <? snd x))
  ||  ((fst y <? fst x) && (fst x <? snd y)).
Definition range : Type := var * (nat * nat).
Definition range_intersect (x y : range) :=
  ((fst x) =? (fst y)) && nat_range_inter (snd x) (snd y).

(*
Definition same_name (x y:range) := fst x =? fst y.
*)
(* range is: (qubit_id, (lo, hi)) *)
...
(* assuming you already have:
   nat_range_inter : (nat * nat) -> (nat * nat) -> bool
*)
...
Fixpoint intersect' (x : range) (y : locus) : bool :=
  ...
  | a :: yas =>
      if same_name x a then
        if nat_range_inter (snd x) (snd a) then true else intersect' x yas
      else
        intersect' x yas
  end.
(*
